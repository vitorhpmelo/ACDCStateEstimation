...
for a in [1e-4]
    df_errors, df_conv,df_prior = run_fase_experiment_stationary_timebench(N, n0, a, min_sigma, result, data_wls_sr, d_prec, d_keys, d_meas_set, nlp_optimizer_hyb, nlp_optimizer_scada, nlp_optimizer_pmu, ipot_out;cutoff=1e0,se_objective=se_objective)
    df_errors[!, :a] .= a
    df_conv[!, :a] .= a
