export solve_state_and_topology

function solve_state_and_topology end

function solve_state_and_topology(file::String, model_type::Type, optimizer; kwargs...)
    data = _PMMCDC.parse_file(file)
    return solve_state_and_topology(data, model_type, optimizer; ref_extensions=[_PMMCDC.add_ref_dcgrid!], kwargs...)
end

function solve_state_and_topology(data::Dict{String,Any}, model_type::Type, optimizer; kwargs...)
    ...
    return _PM.solve_model(data, model_type, optimizer, build_joint_state_topology; ref_extensions=[_PMMCDC.add_ref_dcgrid!], kwargs...)
end

function build_joint_state_topology(pm::_PM.AbstractPowerModel)

    ## TOPOLOGY VARIABLES
    # PowerModels variables
    variable_dc_branch_state(pm)
    variable_ac_branch_state(pm)
